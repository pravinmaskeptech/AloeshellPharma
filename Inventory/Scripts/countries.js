...
//For Syncfusion Autocomplete
function populateStates(countryElementId, stateElementId) {
    debugger;
    var selectedCountryIndex = $('#' + countryElementId + '').val();
    if (selectedCountryIndex == null || selectedCountryIndex == "") {
    }
    else{
        var stateElement = document.getElementById(stateElementId);      

        var AIndex = country_arr.indexOf(selectedCountryIndex);
        if (AIndex == -1) {
            toastr.error("Country does not exist");
            $('#' + countryElementId + '').val("");
            return;
        }
        AIndex = AIndex + 1;
        var state_arr = s_a[AIndex].split("|");

        $('#' + stateElementId + '').ejAutocomplete({
            watermarkText: "Select State",
            showPopupButton: true,
            dataSource: state_arr
        });
    }
}
...
function populateCountries(countryElementId, stateElementId) {    
    ...
    $('#' + countryElementId + '').ejAutocomplete({
        watermarkText: "Select Country",
        showPopupButton: true,
        dataSource: country_arr
    });   
...
function CheckState(countryElementId, stateElementId) {
    debugger;
    var countryElement = $('#' + countryElementId + '').val();
    var StateElement = $('#' + stateElementId + '').val();

    var AIndex = country_arr.indexOf(countryElement);
    AIndex = AIndex + 1;     
    var state_arr = s_a[AIndex].split("|");
    var StateIndex = state_arr.indexOf(StateElement);
    if (StateIndex == -1) {
        toastr.error("State does not exist");
        $('#' + stateElementId + '').val("");
        return;
    }
}
